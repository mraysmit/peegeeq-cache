...
Raw failure diagnostics and screenshots belong under module `target/` directories. The complete
passing screenshot catalogue is also published to the UI's ignored `docs/screenshots/` gallery.
Screenshots capture the actual development UI without masks, redaction, text suppression, or
replacement values. Native password controls and application reveal/hide states remain unchanged.
Screenshot inspection checks the UI's state, especially after hiding or cleanup; it is not a reason
to obscure content that is legitimately visible. Stable test names, Failsafe XML, and the implementation
plan provide the durable evidence index. The suite does not claim video, trace, or visual-baseline
evidence that it does not generate.
...
- the merged catalogue declares 557 independently reported Java Playwright scenarios, including 18 independent setup-capability degradation cases and 13 isolated product journeys against real TLS PostgreSQL. Ordinary scenarios reuse one suite-scoped registered setup, while setup/authentication/capability/shutdown lifecycle scenarios retain explicit isolation; three additional Failsafe checks validate runnable-artifact and evidence integrity.
...
- no secret or raw sensitive content leaks into storage, URLs, browser/server logs, report text, or notifications; authorized reveals appear in the live UI and direct screenshots, and disappear from the UI/accessibility state after hide or cleanup;
...
- the capability-remediation parent passed a clean 2 September 2026 `mvn -o clean verify` gate across all 11 modules under OpenJDK 25 and PostgreSQL 18.3: 560 Surefire tests, 560 Failsafe tests, and 129 Vitest tests, all with zero failures, errors, or skips; its Failsafe total comprised 557 Java Playwright scenarios and three runnable-artifact/evidence checks;
- the 557-scenario catalogue implements 17 independently named journey owners; the separate Surefire accountability contract covers all 60 OpenAPI operations and runtime tracing verifies declared operations from observed browser traffic. Thirteen `ManagementConsoleProductJourneysIT` cases drive the packaged production asset through Chromium against real TLS PostgreSQL;
- the fixture-lifecycle parent subsequently removed authentication and registration calls from ordinary scenario bodies, added an audit-backed exactly-once registration guard, centralized the complete suite on one Playwright/Chromium process, and added a source-policy gate forbidding additional launch sites. That parent passed its then-current 550-scenario catalogue on 3 September; the merged 557-scenario fixture and U11 locator changes subsequently passed the fresh cumulative gate on 5 September;
...
- the pre-capability-expansion 11-module reactor was green against PostgreSQL 15.17, 16.13, 17.11, and 18.3; the active 557-scenario catalogue and post-U11 cross-version parity matrix were freshly verified on all four versions on 5 September.
...
- the merged desktop-only catalogue contains exactly 557 independently identified Playwright scenarios; the prior 559-scenario cumulative result predates removal of ten unsupported mobile/narrow-viewport cases, addition of `PW-BACKEND-001`, and the seven capability-degradation remediations;
- the capability-remediation parent passed its PostgreSQL 18.3 cumulative gate 557/557 on 2 September 2026, including all 18 capability-degradation paths; the merged shared-fixture and U11 locator state passed its fresh cumulative gate on 5 September; and
- the historical PostgreSQL 15-18 evidence above remains the U10 pre-expansion matrix, superseded for current acceptance by the post-U11 matrix below.
...
**Status:** COMPLETE — U11.0 RED recorded; U11.1-U11.9 GREEN; all five quality guards, 170 UI tests, coverage thresholds, production build, 557-scenario Java Playwright catalogue, complete reactor, leakage review, and PostgreSQL 15-18 matrix GREEN on Windows, 5 September 2026
...
U11.2 evidence, 3 September 2026 (GREEN in the UI module at slice completion; final Java Playwright validation recorded below):
...
U11.3 evidence, 3 September 2026 (GREEN in the UI module at slice completion; final Java Playwright validation recorded below):

- RED: `component-library.guard` listed `OverviewPage.tsx`, `OverviewMonitoring.tsx`, `SessionTrendChart.tsx`, and `MonitoringPage.tsx` (no antd import; raw `<table>`, `<button>`, `<svg>`), and `no-test-fakes`/`zod-fixture` listed `overview-page.test.tsx` and `monitoring-page.test.tsx` (Fake client classes, no loopback fixture);
- GREEN: none of those files appear in any guard output; `test/overview-page.test.tsx` 3/3 and `test/monitoring-page.test.tsx` 1/1 now render the pages inside the real store and router against the loopback server with `*Schema.parse` fixtures (no setup → no request; 64-bit decimal strings preserved; Unavailable values with reasons; stale alert with correlation id after a 503 then recovery with a new observed instant and "2 snapshots"; the metrics SSE stream connects, a `runtime.snapshot` frame replaces the cached runtime query without a second REST call, and the stream is closed on unmount); module totals 37 files / 152 tests, 8 expected guard failures remaining; `npm run quality` clean; `vite build` green; jsdom axe scans of Overview and Monitoring report zero violations;
- reference pieces cloned into `src/components/common`: `StatCard` (Card + Statistic, `metric-card` class), `SetupScopeBar` (Row/Col + Select fed by `useListSetupsQuery`, committing scope only after `useLazyGetSetupCapabilitiesQuery` succeeds, so the shell's capability gating stays consistent), `FilterBar` (Card + Input.Search + labelled Selects), `ConfirmDialog` (`Modal.confirm` helpers); `SessionTrendChart` is a Recharts `AreaChart` in a `ResponsiveContainer`; `OverviewMonitoring` is Card + Descriptions + Table with each panel a named `<section>`; `OverviewPage` and `MonitoringPage` read through `useGetOverviewQuery`, `useGetDatabaseMonitoringQuery`, `useGetRuntimeMonitoringQuery`, `useGetActivityQuery` (Overview polls at the refresh preference); the current-session trend moved into the Zustand live store (`recordSnapshot`, the reference's `updateChartData` pattern); Monitoring pushes validated SSE snapshots into the RTK Query cache with `inspectionApi.util.upsertQueryData`;
- `ManagementClients` gained `metricsStream`, `pubSubStream`, and `origin` (from a new `SessionClient.origin` getter) so transports use the same origin as the REST clients in tests; the loopback fixture gained `request.onClose` for stream-teardown assertions; shared formatting helpers moved to `src/presentation/display-format.ts`; decorative antd icons inside labelled buttons carry `aria-hidden` so accessible names stay exact;
- Java locator changes made in the same change set: `ManagementOverviewBrowserIT.metric()` now filters `.metric-card` and asserts the value in `.ant-statistic-content`; `ManagementMonitoringBrowserIT` detail helpers target antd `Descriptions` (`.ant-descriptions-item`, `-label`, `-content`); `ManagementConsoleProductJourneysIT` viewport-containment selectors use `.ant-layout-sider`; the `section(...).containsText("SweeperEnabled")`-style assertions were verified to hold because antd renders the label and content spans adjacent (textContent `SweeperEnabled`).

U11.4 evidence, 3 September 2026 (GREEN in the UI module at slice completion; final Java Playwright validation recorded below):

- RED: `component-library.guard` listed `SetupsPage.tsx` (42 raw control sites), `NamespacesPage.tsx`, and `NamespaceDetailsPage.tsx`; `no-test-fakes`/`zod-fixture` listed `setups-page.test.tsx` and `namespaces-page.test.tsx` (Fake client classes);
- GREEN: none of those files appear in any guard output; `test/setups-page.test.tsx` 6/6 and `test/namespaces-page.test.tsx` 4/4 render against the loopback server with `*Schema.parse` fixtures (setup table, details dialog with `Descriptions`, viewer-only controls, the full registration form round trip with the password proven absent from the DOM and from Redux state, detach confirmation with the CSRF header on the wire and scope cleared, list problem with correlation id; namespace filters and opaque cursor navigation observed as query strings on the wire, export query, details totals and tabs with scope change); module totals 37 files / 152 tests, 8 expected guard failures remaining; `npm run quality` clean; jsdom axe scans of the setups list, the open registration dialog, the namespace list, and namespace details report zero violations;
- implementation: `SetupsPage` on antd `Table`/`Tag`/`Modal`/`Form`/`Input`/`InputNumber`/`Select`/`Checkbox`/`Descriptions`/`List`/`Alert` with `useListSetupsQuery`, lazy details/health/capabilities queries, and the lifecycle mutations (`register`, `testConnection`, `testRegistered`, `connect`, `detach`, `forget`) whose tag invalidations refresh the list without a manual reload; `NamespacesPage` on `Card`+`Form`(inline)+`Select`+`Table` with `useGetNamespacesQuery` keyed by setup, filters, and cursor (`useLazyExportNamespacesQuery` for the download); `NamespaceDetailsPage` on `StatCard` row, antd `Tabs` navigating to sibling resources, and `Descriptions` panels; reviewed filter values live in `namespace-filters.ts`;
- test infrastructure: `test/support/render.tsx` (`renderWithProviders`) wraps pages in the real router, store, and clients with antd `theme.token.motion` disabled, because jsdom never fires transition-end events and an animated Modal would stay at opacity 0 (a design-token configuration, not a double); `zod-fixture.guard` now also accepts fixture factories whose body is a `*Schema.parse` call;
- Java locator changes made in the same change set: new `AntSelect` helper (`choose`/`optionLabels`) because antd `Select` is a combobox with a portal listbox and Playwright's native `selectOption` does not apply; `ManagementNamespaceBrowserIT` status/sort option scenarios assert the reviewed labels (`All`, `Healthy`, `Expired backlog`, `Active locks`; `Namespace`, `Live entries`) and `applyStatus` chooses by label; `ManagementConsolePostgresFixture.registerSetup` chooses `Apply bundled schema at startup` through the helper; `ManagementSetupBrowserIT` reads the details dialog through `.ant-descriptions`.

U11.5 evidence, 4 September 2026 (GREEN in the UI module at slice completion; final Java Playwright validation recorded below):

- RED: `component-library.guard` listed `EntriesPage.tsx`, `EntryDetailsPage.tsx`, and `EntryValueFormatter.tsx` (no antd import; raw `<table>`, `<form>`, `<input>`, `<select>`, `<textarea>`, `<button>`, hand-rolled `Modal`); `no-test-fakes`/`zod-fixture` listed `entries-page.test.tsx`, `entry-details-page.test.tsx`, and `entry-administration-pages.test.tsx` (`FakeEntryClient`, `FakeEntryDetailsClient`, `FakeInspection`, `FakeAdministration` implementing `*ClientPort`);
- GREEN: none of those files appear in any guard output; `test/entries-page.test.tsx` 3/3, `test/entry-details-page.test.tsx` 3/3, and `test/entry-administration-pages.test.tsx` 9/9 render against the shared `test/support/entry-fixture.ts` loopback fixture (every body from `entryMetadataSchema`/`entryPageSchema`/`revealedEntryValueSchema`/`entrySetResultSchema`/`bulkDeletePreviewSchema`/`bulkDeleteResultSchema`) and observe the wire: capability-gated `INCLUDE_EXPIRED` option absent from the open listbox, setup and namespace scope required before any `/entries` request, filter query strings (`prefix`, `valueType`, `ttlState`, `sort`, `limit`) and the opaque cursor stack (`[null, null, 'entry-cursor-2']` — backwards navigation replays the server cursor from the RTK Query cache and never fabricates one), metadata-only rendering with 64-bit versions preserved, viewer-only controls, create with `If-None-Match: *` and the typed JSON contract value, CAS default with `If-Match: "v3"` and the committed version re-read into `Descriptions`, upsert without a precondition and `REPLACE` TTL only when chosen, `VERSION_MISMATCH` conflict preserving the operator's input and refetching metadata, TTL/persist/touch/delete all carrying `If-Match` with the observed version, bulk preview of exact-version targets and of the applied server filter, execution only after the typed server phrase, and the reveal lifecycle (POST with CSRF header and optional reason, value absent from the URL, Web Storage, and Redux state, explicit copy and hide, auto-hide after the server window, hide on `visibilitychange`, hide on route or setup change); module totals 37 files / 155 tests (147 passing, the 8 remaining failures are the intended U11.0 guard assertions for counters/locks/advanced/pub-sub/settings, the legacy client tests, and `Modal.tsx`); `npm run quality` clean; jsdom axe scans of the entry list, the open TTL-state listbox, the create dialog, the bulk-delete dialog, entry details, the open edit form, the revealed value panel, and the delete dialog report zero violations;
- implementation: `EntriesPage` on antd `Card`/`Form`/`Input`/`Checkbox`/`Table` (`rowKey="encodedKey"`)/`Tag`/`Modal`/`Alert`/`Empty` with `useGetEntriesQuery` keyed by setup, namespace, filters, and cursor, and `useSetEntryMutation`/`usePreviewEntryBulkDeleteMutation`/`useExecuteEntryBulkDeleteMutation`; `EntryDetailsPage` on `Descriptions` (`aria-label="Entry metadata"`), `Form`, `Input.TextArea`, `Modal` with `useGetEntryQuery` and the five entry mutations, the reveal still on `useManagementClients().inspection.revealEntryValue` (no-store client, component memory only, design §8.2); `EntryValueFormatter` on `Button` (`aria-pressed`) and `Tag`; `src/features/entries/entry-filters.ts` holds the reviewed option lists, `cacheValueFor`, `positiveInteger`, and `formatTtl`; new `src/components/common/ValueSelect.tsx` wraps antd `Select` and stamps the server value on a `data-value` attribute of every option and of the rendered selection (`optionRender`/`labelRender`), now also used by the namespace status/sort filters and the setup schema-bootstrap select;
- design decision recorded: the shell `ConfigProvider` (and the test `renderWithProviders`) sets `virtual={false}`; with virtual scrolling rc-select gives `role="option"` only to a hidden three-item accessibility shadow list around the active option, so assistive technology and role-based locators could not see `Replace TTL` or `Include expired`; every console select is a short fixed list, so the non-virtual listbox with a role on every option is the correct antd configuration, not a workaround;
- test infrastructure: `renderWithProviders` now installs the router/store/client providers as the Testing Library `wrapper` so `rerender` keeps them; `chooseOption(user, combobox, label)` drives a select the way an operator does; `startEntryFixture(initial)` returns `{ server, store, sessionClient, state, requests, close }` with mutable `state` (`metadata`, `setEntryProblem`, `autoHideAfterMillis`); `zod-fixture.guard` accepts a shared `test/support/*-fixture` module as the loopback import and no longer counts the transport's own `outgoing.end(JSON.stringify(payload))` as a fixture (every `respond.json` argument is still checked at the call site);
- Java locator changes made in the same change set and validated by the final Windows run: `AntSelect` is now value-based — `choose(page, combobox, value)` clicks `[role='option'] [data-value='…']` in the open dropdown, `values(page, combobox)` returns the server values in display order, `selection(combobox)` locates the rendered selection for `hasAttribute("data-value", …)`, and `optionLabels` is retained for the two namespace scenarios whose expected result is the visible wording; `ManagementEntryInspectionBrowserIT`/`ManagementEntryBulkBrowserIT` `filter()` and `options()` and `ManagementEntryAdministrationBrowserIT` (`#new-entry-type`, `#entry-set-mode`, `#entry-ttl-mode`, `fillCreate`, `save`, `dl[aria-label='Entry metadata']` → `[aria-label='Entry metadata']`) use the helper; `ManagementCapabilityBrowserIT` asserts `INCLUDE_EXPIRED` is absent from `AntSelect.values(...)`; `ManagementNamespaceBrowserIT.applyStatus` passes the server value again and `statusLabel` is deleted; `ManagementConsolePostgresFixture` chooses schema bootstrap `APPLY` by value.

U11.6 evidence, 4 September 2026 (GREEN in the UI module at slice completion; final Java Playwright validation recorded below):

- RED: `component-library.guard` listed `CountersPage.tsx` (raw `<table>`, `<input>`, `<button>`, hand-rolled `Modal`), `LocksPage.tsx`, and `AdvancedOperationsPage.tsx` (raw `<input>`, `<textarea>`, `<button>`); `no-test-fakes`/`zod-fixture` listed `counter-lock-pages.test.tsx` (`CountersFake`, `LocksFake`) and `advanced-operations-page.test.tsx` (`BackendFake`);
- GREEN: none of those files appear in any guard output; `test/counter-lock-pages.test.tsx` 9/9 and `test/advanced-operations-page.test.tsx` 4/4 render against the shared `test/support/resource-fixture.ts` loopback fixture (mutable counter and lock state so committed mutations are visible to the next read; every body from `counterSchema`/`counterPageSchema`/`lockSchema`/`lockPageSchema`/`revealedLockOwnerSchema`/`bulkDeletePreviewSchema`/`bulkDeleteResultSchema` and the backend-capability result schemas) and observe the wire: viewer read-only surfaces with no non-GET request, live trimmed filters as `namespace`/`prefix` query strings that also clear the selection, `COUNTER_OVERFLOW` from the server shown as an alert with no status, a committed `-2` adjustment with `If-Match: "v3"` re-read into the table, delete with the exact observed version, create by exact value with `If-None-Match: *` and by signed adjustment with `createIfMissing`/`REPLACE` TTL, bulk preview of exact-version targets and execution only with the typed `DELETE 1 COUNTERS` phrase, the lock dialog re-reading the lock before display and again before release (`If-Match: "v5"` after the fixture advanced the version), owner reveal through the no-store client with the token absent from Redux and Web Storage and hidden on explicit hide and `visibilitychange`, a stale release surfacing `VERSION_MISMATCH` without releasing, and the advanced workspace sending existence/scan/batch-get/batch-set/batch-delete/metrics/acquire/renew/ownership/release with the typed contract bodies, rejecting malformed batch input locally before any request, withholding capability-gated panels, and clearing the owner token; jsdom axe scans of the counter list, manage dialog, create dialog, bulk dialog, lock list, manage dialog with the owner revealed, release dialog, and the advanced workspace with results report zero violations;
- implementation: `CountersPage` on antd `Card`/`Form`/`Input`/`InputNumber` (`stringMode`, `controls={false}`, so the exact signed 64-bit decimal string reaches `counterSetBodySchema`/`counterAdjustBodySchema` without ever becoming a JavaScript number)/`Checkbox`/`Table` (`rowKey` from the encoded identifiers)/`Modal`/`Alert`/`Empty` with `useGetCountersQuery` and the seven counter mutations; `LocksPage` on `Table`/`Modal`/`Form` with `useGetLocksQuery`, `useLazyGetLockQuery` (cache bypassed: `trigger(ref, false)`) for the fresh read before display and before release, `useForceReleaseLockMutation`, the release dialog focusing "Confirm lock key" via `afterOpenChange`, and the owner reveal on `useManagementClients().resource.revealLockOwner`; `AdvancedOperationsPage` on `Card`/`Form`/`Row`/`Col`/`Input`/`Input.Password` (owner token, no visibility toggle)/`Input.TextArea`/`Checkbox`/`Descriptions` (exact metrics) with every call on the no-store `backendCapability` client — nothing on that page enters RTK Query by design (§8.2); `ManagementShell` no longer passes `client=` to any page and the retained `sessionClient` prop is deleted;
- test infrastructure: `test/support/loopback-server.ts` `close()` now calls `server.closeAllConnections()` after `server.close()`, because undici's keep-alive pool and a reconnecting SSE client otherwise hold the listener open past the test (the cause of an `afterEach` hook timeout while converting the Pub/Sub page); `test/overview-page.test.tsx` awaits each independently loaded Overview section (`findBy*`) instead of asserting them synchronously after the first, removing a load-dependent flake seen once in the full run;
- Java locator changes: none required — `ManagementCounterBrowserIT`, `ManagementLockBrowserIT`, `ManagementCapabilityBrowserIT`, and the advanced-workspace journey address the pages by role, label, `thead th`/`td` order, dialog names, `strong` phrase, and `data-theme`, all preserved (column order Selection/Namespace/Key/Value/Version/Updated/TTL/Actions and Namespace/Key/Fencing token/Version/Lease expires/Remaining/Owner/Actions; `Exists: Yes`, `Lock result:`, `Cache Sets`, region names `Scan result`/`Batch get result`/`Batch set result`/`Batch delete result`; dialogs `Manage {key}`, `Create counter`, `Confirm counter deletion`, `Release current lock version?` removed from the DOM on close through `destroyOnHidden`).

U11.7 evidence, 4 September 2026 (GREEN in the UI module at slice completion; final Java Playwright validation recorded below):

- RED: `component-library.guard` listed `PubSubPage.tsx` (raw `<table>`, `<input>`, `<textarea>`, `<button>`) and `SettingsPage.tsx` (five raw `<select>`); `no-test-fakes` listed `pubsub-page.test.tsx` (`PubSubFake`, `StreamFake` implementing `PubSubClientPort`/`PubSubStreamPort`) and the `client:`/`stream:` props on `PubSubPage`; `zod-fixture` listed `pubsub-page.test.tsx` and `settings-page.test.tsx` as component tests without the loopback fixture;
- GREEN: none of those files appear in any guard output; `test/pubsub-page.test.tsx` 4/4 renders against a loopback server that serves the subscription, the live SSE stream (`event: pubsub.message` frames over a kept-open response, closed and counted per fixture), publish, delete, and the no-store payload reveal, observing: viewer surfaces without publish or reveal, the subscription body and `Accept: text/event-stream` on the wire, the message metadata arriving through the SSE transport into the Zustand live store, reveal held only in component memory (absent from Redux and the live store) and hidden on explicit hide, `visibilitychange`, and stop, stop deleting the subscription and closing the stream (`closed === opened === 1`), publish carrying the CSRF header and never described as delivery, and UTF-8 byte limits for channel and payload enforced before transport; `test/settings-page.test.tsx` 2/2 drives every preference through the antd Selects and asserts the allowlisted record in `localStorage`, five `peegeeq:preferences-changed` events, no request leaving the browser, and the Descriptions values (`LOCAL_TOKEN`, active setup, limits, migration version); jsdom axe scans of the subscribe form, the subscribed page with a revealed payload, Settings, and the open Theme listbox report zero violations;
- implementation: `PubSubPage` on `Card`/`Form`/`Input`/`Input.TextArea`/`Table`/`Tag`/`Alert` with `useCreateSubscriptionMutation`/`usePublishMessageMutation`/`useDeleteSubscriptionMutation`, the SSE transport connected through `clients.pubSubStream` at `${clients.origin}${streamPath}` and writing to the new live-store slice (`pubSubConnection`, `pubSubMessages`, `setPubSubConnection`, `receivePubSubMessage(message, bufferLimit)`, `clearPubSubMessages`, `stopPubSub`) — metadata only, so `sensitive-dto.guard` stays green — and payload reveal on `useManagementClients().pubSub.revealPayload`; `SettingsPage` on `Card`/`Descriptions`/`Form`/`ValueSelect` with the reviewed option lists in `src/features/settings/settings-options.ts` (numeric preferences carried as their string contract values and parsed back through the allowlisting `savePreferences`);
- Java locator changes made in the same change set and validated by the final Windows run: `ManagementMonitoringBrowserIT` Theme scenario and `selectAndVerifyStored` use `AntSelect.choose(page, combobox, value)`; its Settings case 27 reads `.ant-descriptions-item-content` instead of `dd` (the `detailRow` helper already addressed `.ant-descriptions-item` since U11.3, so Settings cases 23-27 become satisfiable by this slice); `ManagementShellBrowserIT` timezone and refresh-interval persistence scenarios choose by value and assert `AntSelect.selection(...)` `data-value` after reload; no `selectOption` call remains in the browser suite.

U11.8 evidence, 4 September 2026 (GREEN in the UI module):

- RED: `no-test-fakes.guard` listed `vi.stubGlobal('fetch')` in `monitoring-live.test.ts` and `pubsub-live.test.ts`, `vi.fn` in `preferences.test.ts`, and the twelve `*ClientPort`/`*StreamPort`/`MonitoringSocketPort` seams across `src/api`; `zod-fixture.guard` listed `app.test.tsx` (shell rendered without the loopback fixture) and thirteen served bodies in `backend-capability-client`, `entry-administration-client`, `inspection-client`, `session-client`, and `setup-client` tests that were hand-written JSON on private `node:http` servers;
- GREEN: every `test/*.test.ts(x)` now serves through `test/support/loopback-server.ts` (or a shared `*-fixture` built on it) with `*Schema.parse` bodies and explicit `invalidBody(...)` negatives, and no test file imports a vitest mocking, stubbing, spying, or timer API; `test/app.test.tsx` 5/5 renders the shell against the loopback (navigation, identity, role badge, connection state, theme persisted without the CSRF token, capability-gated route, restored scope re-reading `/capabilities`) and, new, mounts the real `App` under `/ui` to drive the local bootstrap gate end to end — 401 on load, a rejected token surfacing `BOOTSTRAP_TOKEN_INVALID` with the field cleared, the accepted token exchanged on the wire, the shell rendered with neither the token nor the CSRF proof in the DOM, and `End local session` sending `DELETE /api/v1/session/local` with the `X-PeeGeeQ-CSRF` header; `monitoring-live` and `pubsub-live` observe `CONNECTING → CONNECTED → STALE` against a stream the loopback server ends after the handshake and assert `Accept: text/event-stream` on the wire; `preferences.test.ts` asserts observable state (the dispatched event type, the stored allowlist, defaults on unreviewed or unreadable records); the five client tests keep every previous assertion and add the precondition, problem, unreadable-body, non-problem-failure, unauthorized-clears-CSRF, and cacheable-sensitive-response paths that were unreachable before; all `*Port` interfaces, `implements` clauses, and the `client=`/`stream=` page props are deleted;
- guard corrections made while converting: `zod-fixture.guard` binds identifiers only from single-line declarations (`let body: unknown = xSchema.parse(...)`, previously a multi-line type annotation let `let server: LoopbackServer;` swallow the next declaration) and checks only the first argument of `respond.json(status, body, headers)`;
- coverage: `src/api/**` branches 83.58% and `src/state/**` branches 92.31% (thresholds 80), module total 85.16% branches, 169 tests in 36 files, `npm run test:coverage` exit 0.

U11.9 evidence, 4 September 2026 (GREEN in the UI module at slice completion; final Java Playwright validation recorded below):

- RED: `component-library.guard` `src/components/Modal.tsx (the hand-rolled dialog) no longer exists` was the last failing guard assertion;
- GREEN: `src/components/Modal.tsx` and `test/modal.test.tsx` are deleted; `src/components/common/ConfirmDialog.tsx` and `FilterBar.tsx` are deleted as unused (the setups page keeps a controlled antd `Modal` so each confirmation dialog carries its exact accessible name — `Modal.confirm` cannot guarantee that contract — and the namespace filter is an inline antd `Form` in a `Card`); `src/styles/foundation.css` shrinks from 838 to 455 lines by deleting every rule antd now owns — native `button`/`input`/`select` resets and focus rings, `.button*`, `.field*`, `.modal`/`.modal-backdrop`/`.modal--compact`/`.modal__heading`, `.data-table*`, `.tabs*`, `.badge*`, `.empty-state`, `.callout`/`.notice`, `.muted`, `.metric-grid`, `.overview-panels`, `.panel-problem`, the hand-drawn `.trend-chart*`/`.trend-legend*`, and the `dl`-era `.details-list*`/`.compact-details*` rules (the `Descriptions` components no longer carry those class hooks) — while keeping the theme root, the shell layout (`.console*`, `.workspace*`, `.table-scroll`, `.pagination`, `.form-grid`, `.filter-bar`, `.metric-card` which `ManagementOverviewBrowserIT` addresses, `.value-content`, `.json-tree`, `.sr-only`) and the responsive block; `npm ls --depth=0` is consistent and the `every declared production dependency is imported by src/` guard is green (`@ant-design/icons`, `@reduxjs/toolkit`, `antd`, `react`, `react-dom`, `react-redux`, `react-router-dom`, `recharts`, `zod`, `zustand`);
- final UI-module totals: all five `test/quality` guard files GREEN (17 assertions); `npm run quality` clean; `vite build` green; 36 test files / 169 tests passing; branch coverage `src/api` 83.58%, `src/state` 92.31%, `src/store` 95.30%, `src/app` 89.76%, `src/features` 82.85%, total 85.16%;
- merge-resolution evidence (Windows workstation, 4 September 2026): the eight conflicted paths are staged with no unmerged index entries; `ManagementConsolePostgresFixture` combines the suite-shared environment with the capability-source filter; `mvn -o -pl :peegee-cache-rest -am -DskipTests test-compile` is green across eight modules; and `ManagementBrowserCoverageTest` passes 2/2, confirming 60 OpenAPI operations and 557 scenarios;
- then-outstanding for phase closure (owner: Windows workstation): run the 557-scenario Playwright catalogue with the U11.2-U11.7 locator changes, followed by the complete reactor and the PostgreSQL 15-18 matrix; this was completed by the 5 September acceptance evidence below.

Current-working-tree Windows remediation and UI re-gate, 5 September 2026 (GREEN):

- environment diagnosis: the UI dependency tree was restored once with the module-managed npm `ci` (441 packages, zero vulnerabilities), correcting the installed `antd` tree from 5.12.8 to the lockfile/package pin 5.26.5; invoking the managed npm CLI alone was insufficient because Windows npm command shims launched bare `node` from `PATH`, which resolved the unsupported workstation Node 24.11.1 instead of the module pin Node 22.22.2;
- RED: repeated full-coverage runs under that unsupported Node 24 process produced four intermittent Pub/Sub/Monitoring SSE failures (166/170), while focused suites stayed green; real loopback fixtures with a first-handshake HTTP 503 made the missing reconnect-state cleanup observable, and the Overview test exposed an independent stale-`datetime` assertion race;
- GREEN: the Pub/Sub page now separates operational status from transient live-transport notices, Pub/Sub and Monitoring clear interruption notices on `CONNECTED`, the loopback tests require recovery from a controlled first-handshake 503 before the server closes the stream, and Overview waits for the timestamp attribute to change; no mocks, stubs, fake timers, client ports, or replacement UI implementations were introduced;
- deterministic execution: component tests use the Vitest thread pool with at most four workers because each worker owns real loopback HTTP/SSE fixtures; `preverify` rejects any runtime other than Node 22.22.2 and reports the executable mismatch; on Windows the module's `node` directory must be prepended to `PATH` before npm is launched so child shims inherit the pin;
- command: from `peegee-cache-management-ui`, `$uiNodeDir = (Resolve-Path '.\\node').Path; $env:Path = "$uiNodeDir;$env:Path"; & '.\\node\\node.exe' '.\\node\\node_modules\\npm\\bin\\npm-cli.js' run verify`;
- result: two consecutive full coverage runs and the final exact `npm run verify` gate passed; the final gate reports 36 files / 170 tests, statements 94.18% (5559/5902), branches 86.11% (1569/1822), functions 84.77% (590/696), and lines 94.18% (5559/5902); `src/api/**` reports 97.08% lines/statements, 94.04% functions, and 87.01% branches, while `src/state/**` reports 98.60% lines/statements, 84.62% functions, and 92.31% branches;
- `generate:openapi`, TypeScript, ESLint with zero warnings, all five quality guards, coverage, and the Vite production build are green; the build emits the known non-blocking JavaScript chunk-size advisory, and jsdom/Recharts emit known non-failing layout advisories.

U11 final browser, reactor, compatibility, and hygiene evidence, 5 September 2026 (GREEN):

- the complete Failsafe gate produces 27 XML reports aggregating to 560 tests, zero failures, zero errors, and zero skips: 557/557 reportable browser scenarios plus `ManagementPlaywrightObservationIT` (1) and `ManagementRunnableArtifactIT` (2); the fresh self-contained `playwright-evidence.html` records 557/557 passed and rejects every registered bootstrap-token, database-password, entry-value, lock-owner, and Pub/Sub-payload canary before writing;
- real-browser synchronization corrections use only observable state: exact Setups heading matching; reveal waiting for the `Hide value` state before firing `visibilitychange`; TTL text accepting the browser's one-to-three-decimal countdown; counter headers waiting for all eight cells; setup registration waiting up to the same documented 15-second window as the backend connection attempt; and the two multi-operation real-loopback component workflows receiving a bounded 30-second per-test ceiling while the global 15-second ceiling remains unchanged;
- full 11-module `mvn --batch-mode --no-transfer-progress verify` reactors pass on `postgres:15.17-alpine` (22:23), `postgres:16.13-alpine` (20:21), `postgres:17.11-alpine` (20:38), and `postgres:18.3-alpine` (21:21); every row includes 36/36 UI files, 170/170 UI tests, 560/560 Failsafe tests, benchmark headroom, runnable packaging, and zero failures/errors/skips;
- retained final logs are `target/u11-verification/reactor-postgresql-{15.17,16.13,17.11,18.3}.log`; exploratory failures are separately named `.failed-*` and are not acceptance evidence; the focused clean browser log is `peegee-cache-rest/target/playwright-full-final.log` and the latest matrix-generated evidence is `peegee-cache-rest/target/playwright-evidence.html`;
- final-log review found no failure/error signatures, JVM dumps, crash artifacts, leaked sensitive canaries, Mockito, substitute mocking framework, UI test double, client/transport port seam, or prohibited direct transport. Expected non-blocking output is limited to JDK 25 Maven/Guice `sun.misc.Unsafe` deprecation, idempotent PostgreSQL schema/relation notices, deliberate Pub/Sub connection-loss and telemetry-failure injections, Maven Shade overlap/module-info advisories, the transitive `glob` deprecation, Vite's bundle-size advisory, and jsdom/Recharts layout limitations;
- `git diff --check` is green, the index has no staged change, and generated verification artifacts remain ignored. Phase 8.3 U11 therefore meets every exit criterion in this plan.
...
- the Java Playwright catalogue passes 557/557 with any locator changes recorded per slice, and `ManagementAccessibilityBrowserIT` has no serious or critical axe violations at 1440x900;
...
| U10 Final acceptance | COMPLETE | Deterministic artifacts; 17 named browser journeys with executable ownership/runtime evidence for all 60 OpenAPI operations; merged 557-scenario desktop-only catalogue; and the pre-expansion PostgreSQL 15.17, 16.13, 17.11, and 18.3 matrix are retained |

| U11 Reference parity | COMPLETE (5 September 2026) | Directive/guard gates RED then GREEN; antd/Recharts/RTK Query in use with unused-dependency and fake-free tests proven by guard tests; api/state branch thresholds green; 557/557 Playwright scenarios and all three infrastructure checks green; complete 11-module PostgreSQL 15.17/16.13/17.11/18.3 reactors green |
