...
**Second check (24 September 2026).** A re-check against the checkout found and fixed three further problems: `USER_JOURNEYS_GUIDE.md` linked to two plans that had been archived on 11 September (now linked in `archive/`, next to the UI design and the screenshot-evidence rules); the new development guidelines overstated two points (all data access through `preparedQuery`, where `query()` is in fact used for scripts, fixed read-model queries and `LISTEN`/`UNLISTEN` with an escaped channel identifier; and every PostgreSQL test using `PgTestSupport`, where the REST browser fixtures manage their own container). The facts the verification pass could not confirm from its notes were confirmed in the code: no Mockito in any POM or source, no `CompletableFuture` or `System.getProperty`/`getenv` in the five production libraries, no JDBC, three `Thread.sleep` calls in tests, the `cache.manager.stop_failed` event, the campaign's hard-coded hit ratio and seed, the six infrastructure checks (one observation, two runnable-artifact, three screenshot), PostgreSQL 18.3 in both cited logs, and three calibration files. Every relative link and anchor in the current documents, the archive and the UI module's documents resolves.
