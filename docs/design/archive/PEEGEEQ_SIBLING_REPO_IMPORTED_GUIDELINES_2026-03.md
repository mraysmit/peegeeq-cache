> **Archived 24 September 2026.** This is the guide imported from the sibling PeeGeeQ queue repository in March 2026 and formerly kept at `docs/guidelines/PEEGEEQ_CACHE_DEV_GUIDELINES.md`. It is not normative for peegee-cache: several of its rules (tag profiles such as `-Pintegration-tests`, mocking in unit tests, `postgres:15.13-alpine3.20`, blocking `join()` in tests, silent `onFailure` handlers) contradict this repository. The current guide is [`docs/guidelines/PEEGEEQ_CACHE_DEV_GUIDELINES.md`](../../guidelines/PEEGEEQ_CACHE_DEV_GUIDELINES.md). Retained as historical reference only.
...
- `PEEGEEQ_CACHE_TEST_COMMANDS.md`: the exact Maven and npm commands for this reactor, `Tee-Object` log naming, rebuild-before-verify, scoped-versus-gate rules, and how to report a run; the root `CLAUDE.md` carries the same workflow rules
