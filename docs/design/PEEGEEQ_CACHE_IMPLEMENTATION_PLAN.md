...
**Status:** **COMPLETE (M0–M11 COMPLETE; CAPABILITY REGATE IN PROGRESS)** — the management backend includes the complete typed/PostgreSQL REST surface, security and lifecycle, atomic administration, audited pub/sub, bounded live transports, mandatory bounded Micrometer/Prometheus telemetry, shared per-setup metrics sampling, both executable authentication configurations, a verified executable Java 21 artifact, operational guidance, complete runtime configuration, and full backend-facade REST parity. The clean 2 September 2026 PostgreSQL 18.3 baseline passed all 11 modules under OpenJDK 25 with 799 Surefire, 553 Failsafe, and 129 Vitest tests and zero failures, errors, or skips. That baseline comprised 550 packaged desktop-browser scenarios and three runnable-artifact/evidence checks. The active catalogue is 557 after adding seven missing independent capability-degradation cases; their focused 18-case gate is green and the complete regate is in progress. The prior PostgreSQL 15.17, 16.13, 17.11, and 18.3 compatibility matrix is retained.
...
| Phase 8: V2 and later | COMPLETE | Phase 8.1 write-behind, Phase 8.2 management backend M0-M10, and Phase 8.3 management UI U0-U10 are complete. Packaged Chromium/PostgreSQL acceptance and the post-change PostgreSQL 15-18 reactor matrix are green. | None |
...
Current baseline evidence includes the clean 2 September 2026 11-module PostgreSQL 18.3 reactor with 799 Surefire, 553 Failsafe, and 129 Vitest tests under OpenJDK 25 and zero failures, errors, or skips. The active 557-scenario capability-remediated catalogue has passed its focused 18-case gate and is undergoing a complete regate. The earlier complete-reactor PostgreSQL 15.17, 16.13, 17.11, and 18.3 compatibility matrix is retained. The authoritative behavior and evidence are in `PEEGEEQ_CACHE_MANAGEMENT_API_IMPLEMENTATION_PLAN.md` and `PEEGEEQ_CACHE_MANAGEMENT_UI_IMPLEMENTATION_PLAN.md`.
...
No Phase 8.3 product implementation task remains. The active desktop catalogue contains 557 independently reported scenarios and three runnable-artifact/evidence checks. Its 550-scenario baseline passed the full-reactor PostgreSQL 18.3 gate on 2 September 2026; the seven-case capability remediation is focused-green and the complete regate is in progress. The prior PostgreSQL 15.17, 16.13, 17.11, and 18.3 compatibility evidence is retained, while a fresh cross-version parity regate remains release validation. Production-topology benchmarking and credentialed Maven Central publication remain intentionally deferred release-readiness actions.
