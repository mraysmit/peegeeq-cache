...
- Statements that carry caller-supplied values use parameterised `preparedQuery(...)` with `Tuple` arguments; never concatenate keys, namespaces, payloads or other caller input into SQL. `query(...)` is used only for statements without bind values: migration scripts, fixed read-model queries, and `LISTEN`/`UNLISTEN`, where PostgreSQL cannot bind the channel and `PubSubSql` escapes it as a quoted identifier. The configured schema name is the one value interpolated into SQL text.
- Multi-statement scripts (the bootstrap and migrations) run through `query(sql).execute()`, which uses PostgreSQL's simple-query protocol and executes every statement in the script. `PgTestSupport` and `PgSchemaMigrator` both apply the complete `V001` this way, and the PostgreSQL-backed tests then exercise the functions and views defined later in that file. Never pass a multi-statement script to `preparedQuery`: the extended protocol accepts one statement. The sibling repository's "multi-statement SQL bug" postmortem is about that distinction; it does not mean `query()` drops statements.
...
- No mocking library is used anywhere in the reactor, and none may be added. PostgreSQL-backed tests use a real Testcontainers database, normally through `PgTestSupport` and `SharedPostgresContainerManager` in `peegee-cache-test-support` (the REST browser fixtures manage their own container). The image defaults to `postgres:18.3-alpine` (`PostgreSQLTestConstants.DEFAULT_POSTGRES_IMAGE`) and is overridden with `-Dpeegeeq.test.postgres.image=...` for the PostgreSQL 15–18 matrix.
