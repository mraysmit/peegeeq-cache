...
export function LocksPage({ canOperate, canReveal, client, selectedSetupId }: { readonly canOperate: boolean; readonly canReveal: boolean; readonly client: LockClientPort; readonly selectedSetupId?: string }) {
  ...
  return <Workspace>{problem !== undefined && <div className="diagnostics" role="alert"><strong>{problem.code}</strong><p>{problem.message}</p></div>}{status !== '' && <p role="status">{status}</p>}{page === undefined ? <p aria-busy="true">Loading locks…</p> : page.items.length === 0 ? <p>No active locks.</p> : <div aria-label="Active locks results" className="table-scroll" role="region" tabIndex={0}><table className="data-table"><thead><tr><th>Namespace</th><th>Key</th><th>Fencing token</th><th>Version</th><th>Lease expires</th><th>Remaining</th><th>Owner</th>{canManage && <th>Actions</th>}</tr></thead><tbody>{page.items.map((item) => <tr key={`${item.encodedNamespace}:${item.encodedKey}`}><td>{item.namespace}</td><td>{item.key}</td><td>{item.fencingToken}</td><td>{item.version}</td><td>{formatDisplayInstant(item.leaseExpiresAt)}</td><td>{item.leaseRemainingMillis} ms</td><td>Masked</td>{canManage && <td><button className="button button--quiet" disabled={busy} onClick={() => void manage(item)} type="button">Manage {item.key}</button></td>}</tr>)}</tbody></table></div>}
