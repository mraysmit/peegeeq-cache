...
export function PubSubPage({ canOperate, canReveal, client, maximumChannelBytes, maximumPayloadBytes, selectedSetupId, stream = defaultStream }: {
  ...
  const [publishChannel, setPublishChannel] = useState(''); const [payload, setPayload] = useState('');
  ...
  const channelBytes = utf8Bytes(channel); const publishChannelBytes = utf8Bytes(publishChannel); const payloadBytes = utf8Bytes(payload);
  ...
  const publish = async () => { if (selectedSetupId === undefined) return; setBusy(true); setProblem(undefined); try { const accepted = await client.publish(selectedSetupId, publishChannel, payload); setStatus(`Accepted by PostgreSQL at ${formatDisplayInstant(accepted.publishedAt)}.`); setPayload(''); } catch (failure) { setProblem(asError(failure)); } finally { setBusy(false); } };
  ...
  return <Workspace>
    ...
    {subscription === undefined ? <section className="details-section" aria-labelledby="subscribe-title"><h2 id="subscribe-title">Subscribe</h2><p>Subscriptions are non-durable and bounded. They end when this console session or setup closes.</p><label className="field" htmlFor="pubsub-channel">Channel<input id="pubsub-channel" maxLength={maximumChannelBytes} onChange={(event) => setChannel(event.target.value)} value={channel} /></label><p>{channelBytes} / {maximumChannelBytes} UTF-8 bytes</p><label className="field" htmlFor="pubsub-buffer">Buffer entries<input id="pubsub-buffer" max={500} min={1} onChange={(event) => setBufferLimit(Number(event.target.value))} type="number" value={bufferLimit} /></label><button className="button" disabled={busy || channelBytes < 1 || channelBytes > maximumChannelBytes || bufferLimit < 1 || bufferLimit > 500} onClick={() => void start()} type="button">Start subscription</button></section> : <section className="details-section" aria-labelledby="subscription-title"><h2 id="subscription-title">Subscription: {subscription.channel}</h2><p>Non-durable · {connection} · bounded to {subscription.bufferLimit} messages</p><button className="button button--danger" disabled={busy} onClick={() => void stop()} type="button">Stop subscription</button><div className="table-scroll"><table className="data-table"><thead><tr><th>Message</th><th>Received</th><th>Bytes</th><th>Payload</th>{canReveal && <th>Actions</th>}</tr></thead><tbody>{messages.map((message) => <tr key={message.messageId}><td>{message.messageId}</td><td>{formatDisplayInstant(message.receivedAt)}</td><td>{message.payloadBytes}</td><td>Masked</td>{canReveal && <td><button className="button button--quiet" disabled={busy} onClick={() => void reveal(message)} type="button">Reveal payload {message.messageId}</button></td>}</tr>)}</tbody></table></div>{messages.length === 0 && <p>No retained message metadata.</p>}</section>}
    {revealed !== undefined && <section className="details-section" aria-labelledby="payload-title"><h2 id="payload-title">Revealed payload</h2><pre className="value-content">{revealed.payload}</pre><button className="button" onClick={() => setRevealed(undefined)} type="button">Hide payload</button></section>}
    {canOperate && <section className="details-section" aria-labelledby="publish-title"><h2 id="publish-title">Publish</h2><p>Success means accepted by PostgreSQL; it does not prove subscriber delivery.</p><label className="field" htmlFor="publish-channel">Publish channel<input id="publish-channel" maxLength={maximumChannelBytes} onChange={(event) => setPublishChannel(event.target.value)} value={publishChannel} /></label><p>{publishChannelBytes} / {maximumChannelBytes} channel UTF-8 bytes</p><label className="field field--wide" htmlFor="publish-payload">Payload<textarea id="publish-payload" onChange={(event) => setPayload(event.target.value)} value={payload} /></label><p>{payloadBytes} / {maximumPayloadBytes} UTF-8 bytes</p><button className="button" disabled={busy || publishChannelBytes < 1 || publishChannelBytes > maximumChannelBytes || payloadBytes > maximumPayloadBytes} onClick={() => void publish()} type="button">Publish</button></section>}
  </Workspace>;
}
