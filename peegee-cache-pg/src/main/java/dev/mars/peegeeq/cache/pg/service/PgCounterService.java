...
public final class PgCounterService implements CounterService {
    ...
    public Future<Long> incrementBy(CacheKey key, long delta) {
        return metrics.observe(CacheOperation.COUNTER_INCREMENT,
                () -> wrapStoreFailure("incrementBy", repository.increment(key, delta, DEFAULTS)
                .map(result -> {
    ...
    public Future<Long> decrementBy(CacheKey key, long delta) {
        return metrics.observe(CacheOperation.COUNTER_DECREMENT, () -> {
            if (delta < 0) {
    ...
    public Future<Optional<Long>> getValue(CacheKey key) {
        return metrics.observe(CacheOperation.COUNTER_GET,
                () -> wrapStoreFailure("getValue", repository.get(key)));
    }

    @Override
    public Future<Long> setValue(CacheKey key, long value, CounterOptions options) {
        return metrics.observe(CacheOperation.COUNTER_SET, () -> {
            try {
    ...
    public Future<TtlResult> ttl(CacheKey key) {
        return metrics.observe(CacheOperation.COUNTER_TTL,
                () -> wrapStoreFailure("ttl", repository.ttl(key)));
    }

    @Override
    public Future<Boolean> expire(CacheKey key, Duration ttl) {
        return metrics.observe(CacheOperation.COUNTER_EXPIRE, () -> {
            try {
    ...
    public Future<Boolean> persist(CacheKey key) {
        return metrics.observe(CacheOperation.COUNTER_PERSIST,
                () -> wrapStoreFailure("persist", repository.persist(key)));
    }

    @Override
    public Future<Boolean> delete(CacheKey key) {
        return metrics.observe(CacheOperation.COUNTER_DELETE, () -> wrapStoreFailure("delete", repository.delete(key)
                .map(deleted -> {
